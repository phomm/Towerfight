{
\file     PathFind.pas
\author   George Judkin aka GEO
\url      http://delphikingdom.com/asp/viewitem.asp?catalogid=1127
\brief    Pathfinding wave algorithm
\date     28-03-2005

\modified 16-09-2013
\author   phomm
\brief    added unified internal pathint type, added IsPointOnPath function
}
...
type
  ...
  TPathInt = Smallint; // using internal int type. Modified by phomm 16-09-2013

  TPathMapCell = record   // Клетка карты пути в которой мы отмечаем:
    Distance : TPathInt;   // - порядковый номер клетки в маршруте и
    Direction : TPathInt;  // - направление, с которого приехали на эту клетку.
  end;
  ...
  TGetCostFunc = Function(X,Y,Direction : TPathInt) : TPathInt
    of object; // used for methods. Modified by phomm 16-09-2013

// Added by phomm 16-09-2013
function IsPointOnPath(const Point: TPoint; const Path: TPath;
  var Idx: Integer): Boolean; {$IFDEF PASCAL_INLINE}inline;{$ENDIF}
...
function MakePathMap(
  MapWidth,MapHeight : TPathInt;
  StartX,StartY : TPathInt;
...
function FindPathOnMap(const PathMap : TPathMap; X,Y : TPathInt) : TPath;

{Функция ищет путь из точки (StartX,StartY) в точку (StopX,StopY).}
function FindPath(
  MapWidth,MapHeight : TPathInt;
  StartX,StartY,StopX,StopY : TPathInt;
  GetCostFunc : TGetCostFunc) : TPath;
function DirToDX(Direction : TPathInt) : TPathInt;
function DirToDy(Direction : TPathInt) : TPathInt;
...
type
  TWaveCell = record      // Одна ячейка фронта волны. Содержит:
    X,Y : TPathInt;        // - координаты точки, в которую мы движемся;
    Cost : TPathInt;       // - оставток времени на движение до точки;
    Direction : TPathInt;  // - напрвление, по которому мы движемся.
  end;

  TWave = class
  private
    FData : array of TWaveCell;
    FPos : TPathInt;                // Текущая позиция в массиве.
    FCount : TPathInt;              // Количество реальных элементов в массиве.
  ...
  public
    ...
    property MinCost : TPathInt read FMinCost;  // Минимальное значения Cost
                                               // среди всех элементов массива.
    constructor Create;
    destructor Destroy; override;
    procedure Add(NewX,NewY,NewCost,NewDirection : TPathInt); // Добавляет новый
                                                             // элемент в массив.
...
var
  GetCostFunc : TGetCostFunc;  // Переменная для хранения функции определения
                               ...
                               // программы.
  MapWidth : TPathInt;          // Ширина карты.
  MapHeight : TPathInt;         // Высота карты.
...
procedure TWave.Add(NewX,NewY,NewCost,NewDirection : TPathInt);
begin
...
function GetCost(X,Y,Direction : TPathInt) : TPathInt;
begin
...
function FillPathMap(X1,Y1,X2,Y2 : TPathInt) : TPathMap;
var
  ...
  var                        // запоняет для каждой клатки поле Distance
    X,Y : TPathInt;           // (номер клетки в маршруте) значениями -1,
...
begin
  Result:=NIL;
  if (Length(PathMap) = 0) or (PathMap[Y,X].Distance < 0) then Exit;
...
// Added by phomm 16-09-2013
function IsPointOnPath(const Point: TPoint; const Path: TPath; var Idx: Integer): Boolean;
var
  i: Integer;
begin
  Result := False;
  ...
  for i := 0 to High(Path) do
    if (Point.X = Path[i].X) and (Point.Y = Path[i].Y) then
    begin
      Result := True;
      Idx := i;
      Break; // assume the path has only one exact point without duplicates
    end;
end;
